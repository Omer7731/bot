...
# Function to run the FTP script with a 20-second timeout
def run_ftp_script(bot_token, chat_id, folder_name):
    ftp_script = f"""
    ftp -n 84.228.82.55 <<END_SCRIPT
    quote USER yomedia
    quote PASS 123456Of

    cd /home/yomedia/Desktop/customers/{folder_name}
    binary
    get config.yml /data/data/com.termux/files/home/gramaddict/accounts/{folder_name}/config.yml

    cd /home/yomedia/Desktop/posts
    binary
    get post.txt /data/data/com.termux/files/home/gramaddict/accounts/{folder_name}/urls.txt

    cd /home/yomedia/Desktop/users
    binary
    get users.txt /data/data/com.termux/files/home/gramaddict/accounts/{folder_name}/users.txt
    ...
    try:
        # Run the FTP script with a timeout of 20 seconds
        result = subprocess.run(ftp_script, shell=True, check=True, timeout=20, text=True, capture_output=True)
        print(result.stdout)
        print("FTP script executed successfully.")
    except subprocess.TimeoutExpired:
        message = "FTP script timed out after 20 seconds. No access to the FTP server."
        send_telegram_message(bot_token, chat_id, message)
        print(message)
    except subprocess.CalledProcessError as e:
        message = f"FTP script failed: {e}\n{e.output}"
        send_telegram_message(bot_token, chat_id, message)
        print(message)
...
def main():
    ...
    scheduled_times = ["10:50", "15:24", "21:05", "05:13"]
        ...
        if current_time_unix - last_schedule_check_time >= check_interval_schedule:
            ...
            if current_time in scheduled_times:
                if is_process_running(gramaddict_process.pid):
                    ...
                    # If the time is 05:13, run the FTP script before restarting GramAddict
                    if current_time == "05:13":
                        run_ftp_script(bot_token, chat_id, folder_name)

                    # Restart GramAddict process
                    gramaddict_process = run_gramaddict(folder_name)

...
class PostsViewList:
    ...
    @staticmethod
    def detect_media_type_new(content_desc) -> Tuple[Optional[MediaType], Optional[int]]:
        """
        Detect the nature and amount of a media
        :return: MediaType and count
        :rtype: MediaType, int
        """
        obj_count = 1  # Default to 1 for single media
        if content_desc is None:
            return None, None

        # Missing content description case
        if re.match(r"^,|^\s*$", content_desc, re.IGNORECASE):
            logger.info(
                "That media is missing content description, so I don't know which kind of media it is."
            )
            media_type = MediaType.UNKNOWN

        # Detect carousels with "Photo 1 of X" or "Video 1 of X"
        elif re.search(r"(Photo|Video) \d+ of \d+", content_desc, re.IGNORECASE):
            logger.info("It's a carousel.")
            match = re.search(r"(Photo|Video) \d+ of (\d+)", content_desc)
            if match:
                obj_count = int(match.group(2))  # Extract the total number of items
            media_type = MediaType.CAROUSEL

        # Regular photo detection
        elif re.match(r"^Photo|^Hidden Photo", content_desc, re.IGNORECASE):
            logger.info("It's a photo.")
            media_type = MediaType.PHOTO

        # Regular video detection
        elif re.match(r"^Video|^Hidden Video", content_desc, re.IGNORECASE):
            logger.info("It's a video.")
            media_type = MediaType.VIDEO
        ...
        # Check for carousels with mixed media (photos and videos)
        else:
            carousel_obj = re.finditer(
                r"((?P<photo>\d+) photo)|((?P<video>\d+) video)",
                content_desc,
            ...
            if n_photos + n_videos > 1:
                ...
                logger.info("It's an unrecognized media type or a single media.")
                media_type = MediaType.UNKNOWN
...
class OpenedPostView:
    ...
    def watch_media_from_url(self, media_type: MediaType) -> None:
        """
        Watch media for the amount of time specified in config
        :return: None
        :rtype: None
        ...
        if (
            ...
            if time_left > 0 and media_type != MediaType.REEL:
                logger.info(f"This video is about {time_left}s long.")
                # hardcoded 5 seconds, so we have the time to doing everything without going to the next video, hopefully
                watching_time = min(
                    watching_time,
    ...
    def _is_video_liked(self) -> Tuple[Optional[bool], Optional[DeviceFacade.View]]:
        ...
        """
        logger.info("start us _is_video_liked")
        logger.info(ResourceID.LIKE_BUTTON)
        like_button = self.device.find(
            resourceIdMatches=case_insensitive_re(ResourceID.LIKE_BUTTON)
        )
        logger.info("like_button.exists")
        logger.info(like_button)
        if like_button.exists():
            logger.info("start to get_selected")
    ...
    def like_video_from_file(self) -> bool:
        """
        Like the video with a double click and check if it's liked.
        If double click fails, fallback to clicking the 'heart' icon.
        :return: video has been liked
        :rtype: bool
        """
        sidebar = self.device.find(
        ...
        logger.info("Attempting to like the video with a double click.")
        obj = self.device.find(resourceIdMatches=case_insensitive_re(ResourceID.VIDEO_CONTAINER_AND_CLIPS_VIDEO_CONTAINER))
        ...
        if obj.exists():
            ...
            if not liked:
                logger.info("Double click failed, clicking on the heart icon.")
                if like_button is not None:
                    like_button.click()
                    UniversalActions.detect_block(self.device)
                    liked, _ = self._is_video_liked()
                else:
                    logger.error("No like button found.")
        else:
            logger.error("Video container not found.")

        return liked

    def like_video_from_url(self) -> bool:
        """
        Like the video by clicking the 'heart' icon and check if it's liked.
        :return: video has been liked
        :rtype: bool
        """
        liked = False

        logger.info("Attempting to like the video by clicking the heart icon.")
        obj = self.device.find(resourceIdMatches=case_insensitive_re(ResourceID.VIDEO_CONTAINER_AND_CLIPS_VIDEO_CONTAINER))

        if obj.exists():
            # Attempt to find and click the like button
            liked, like_button = self._is_video_liked()
